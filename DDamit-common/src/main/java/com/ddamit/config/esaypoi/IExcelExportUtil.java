package com.ddamit.config.esaypoi;
 ...
 */
public class IExcelExportUtil {

    public static int USE_SXSSF_LIMIT = 100000;

    public static final String SHEET_NAME = "sheetName";

    private IExcelExportUtil() {
    }
     ...
     * @param entity             表格标题属性
     * @param pojoClass          Excel对象Class
     * @param exportCustomFields 自定义导出字段
     * @param dataSet            Excel对象数据List
     */
    public static Workbook exportExcel(ExportParams entity, Class<?> pojoClass, Map<String, String> exportCustomFields,
                                       Collection<?> dataSet) {
        Workbook workbook = getWorkbook(entity.getType(), dataSet.size());
        new IExcelExportService().createSheet(workbook, entity, pojoClass, exportCustomFields, dataSet);
        return workbook;
    }
    ...
    public static Workbook exportExcel(ExportParams entity, List<ExcelExportEntity> entityList,
        ...
        Workbook workbook = getWorkbook(entity.getType(), dataSet.size());
        ;
        new IExcelExportService().createSheetForMap(workbook, entity, entityList, dataSet);
        return workbook;
    }
    ...
    public static Workbook exportExcel(List<Map<String, Object>> list, ExcelType type) {
        ...
        for (Map<String, Object> map : list) {
            IExcelExportService service = new IExcelExportService();
            service.createSheet(workbook, (ExportParams) map.get("title"),
                    (Class<?>) map.get("entity"), null, (Collection<?>) map.get("data"));
        }
        return workbook;
    }
